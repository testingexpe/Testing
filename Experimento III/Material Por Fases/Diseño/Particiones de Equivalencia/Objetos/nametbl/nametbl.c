                       ...
                       ELEMENTO *p2)
{
  ...
  longitud1 = strlen(p1->nombre);
  longitud2 = strlen(p2->nombre);

  if (longitud1 < longitud2)
     return(strncmp(p1->nombre, p2->nombre, longitud1));
  else return(strncmp(p1->nombre, p2->nombre, longitud2));
}
  ...
  if (strcmp("R_SISTEMA", str_recurso) == 0)
     recurso = R_SISTEMA;
       ...
       recurso = FUNCION;
            ...
            recurso = DATO;
            else if (strcmp("MEMORIA", str_recurso) == 0)
                 recurso = MEMORIA;

  if ((recurso == -1) && (elemento == NULL)) {
    printf("La operacion no se ha podido realizar.\n"); 
  } else if (recurso == -1) {

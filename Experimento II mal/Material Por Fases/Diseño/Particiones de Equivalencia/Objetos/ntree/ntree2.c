                       ...
                       char *dato_hijo)
{
  ...
  if (padre == NULL) {
    printf("No ssse encuentra ningun nodo con clave %s\n", clave_padre);
    resultado = -1;
  }
  else {
    if (padre->n_hijos == padre->max_hijos) {
      espacio = (NODO **)realloc(padre->hijos, (sizeof(NODO *) * (padre->max_hijos*2)));
      assert(espacio != NULL);
      padre->hijos = espacio;
      (padre->max_hijos) *= 2;
    }
  ...
  if (nodo1 == NULL) {
    ...
    if (nodo2 == NULL) {
      ...
      if (nodo1->padre != nodo2->padre)
         hermanos = 0;
      if (hermanos == 1)
        printf("Los nodos %s y %s son hermanos\n", clave1, clave2);
      else printf("Los nodos %s y %s no son hermanos\n", clave1, clave2);
    }
  }
  return resultado;
}
                                 ...
                                 int nivel)
{
  ...
  for (i = 0; i < nodo->n_hijos; i++) {
    imprimir_nodos_arbol(nodo->hijos[i], nivel);
  }
}
 ...
 */
void procesar_comandos(FILE *filep)
{
  char *ptr;
  char buf[BUFSIZ];
